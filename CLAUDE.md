...
| `zxnu_http_bridge.py` | NextSync HTTP bridge: reusable Flask web server (stdlib-only import; Flask loaded on start) republishing a `-listen` session as HTTP routes for the Next's `.http` dot command. Used by the app (Settings toggle, `SETTING_NEXTSYNC_HTTP_BRIDGE`) and `nextsync5.py -http`; docs + call samples in `nextsync/sync/server/HTTP_BRIDGE.md`, e2e test in `test_http_bridge.py` |
