...
; Opens a directory for reading (esxDOS F_OPENDIR $a3). Same calling
; convention as fopen. Returns directory handle (0 on failure). Close it
; with fclose() just like a file handle.
_opendir::
	push	ix
	ld	ix, #0
	add	ix, sp
...
; Reads one directory entry into buf (esxDOS F_READDIR $a4). Returns the
; number of entries read (0 = end of directory, no more entries).
; Entry layout written to buf: +0 = FAT attribute byte (bit 4 = subdir),
; +1.. = ASCIIZ filename, then date/time/size which we ignore.
_readdir::
	push	ix
	ld	ix, #0
	add	ix, sp
	...
	ld	h,  6 (ix)
	push hl
	pop  ix        ; ix = buffer (F_READDIR wants IX)
	ld  b,  #0     ; flags: 0 = standard esxDOS entry
	rst     #0x8
	.db     #0xa4
	ld      hl, #0
	jr      c, readdirfail
	ld      l, a   ; a = entries read (0 = end)
readdirfail:
	pop ix
    ret
