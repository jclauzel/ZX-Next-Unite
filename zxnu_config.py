...
ZX_NEXT_UNITE_DOTN_VERSION = "5.9.13"
...
SETTING_COLOR_RETRO_LOG   = "color_retro_log"       # retro (pygame) log console text colour
SETTING_DESKTOP_THEME     = "desktop_theme"     # "automatic" (default) | "dark" | "custom"
SETTING_UI_LANGUAGE       = "ui_language"       # "en" (default) | "es" | "pt" | "pl" | "ru" | "cs" | "fr"
...
# Onboarding Wizard assistant (zxnu_wizard.py): "false" => hidden (default
# shown), whether the first-run introduction has already been played, and
# the speech-bubble font size in px ("" => the built-in default).
...
# with the .http dot command (port 80, no TLS) can drive the connected Next's
...
SETTING_NEXTSYNC_HTTP_VERBOSE = "nextsync_http_verbose"  # "true"/"false" (default false): trace every bridge request
...
SETTING_NEXTSYNC_RE_LOG_SPLITTER = "nextsync_re_log_splitter_sizes"  # "top,bottom" pane heights (px) of the Remote Explorer explorers ⇄ log splitter (9.7.21)

#: Largest pane size (px) a saved splitter value may claim before it is read
#: as corruption and ignored (9.7.21). No display makes a pane this big, and
#: a number past a C int makes Qt's setSizes raise OverflowError - an
#: ArithmeticError, so the TypeError/ValueError clauses around every restore
#: would let it escape and abandon the rest of that restore.
SPLITTER_MAX_PANE_PX = 100000
...
SETTING_NEXTSYNC_EXTRA_DRIVES  = "nextsync_extra_drives"   # user-declared extra Next drives (e.g. "DE"): the dot only reports C/M/current, extra SD-reader letters are added by hand
...
# Retro 8-bit (pygame) log console text. Matches the classic phosphor green the
# consoles have always used (RetroLogWidget._C_LOG / _C_LOG_GREEN = 120,255,140).
DEFAULT_COLOR_RETRO_LOG = "#78ff8c"
...
def nextsync_verify_crc_enabled(cfg):
    """Settings 'NextSync — Verify CRC …' (9.7.3), decoded the default-ON way:
    a key never saved is "" (the CONFIG_FILE_SETTINGS pre-seed) and an
