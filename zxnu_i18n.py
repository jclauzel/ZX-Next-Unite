...
CATALOGS = {
    "es": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Sesiones: admitir varios Next a la vez (Explorador remoto)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Activado (predeterminado): el servidor NextSync del Explorador remoto admite hasta cuatro Next\nejecutando '.sync5 -listen' a la vez — la lista de máquinas y las pestañas de sesión\neligen cuál controla el panel, y a un quinto se le rechaza con Busy.\nDesactivado: exactamente UNA sesión. Un Next que se conecta mientras otro ocupa la sesión se toma\npor la MISMA máquina que vuelve tras un enlace muerto (su módulo ESP se reinició,\nsu Wi-Fi parpadeó): se cierra el enlace antiguo y el recién llegado ocupa su\nlugar al instante — misma sesión, mismo panel, sin reinicio — en vez de esperar\ndetrás del enlace muerto hasta que caduque. Un comando cortado por la caída se\nreintenta hasta tres veces, con tres segundos de pausa, en cuanto el Next vuelve (el\nregistro muestra retry1, retry2…); el resto de una copia en cola continúa por\nel nuevo. Combínalo con ZX Next Remote 1.2.5 o posterior, cuyo Listener\nvuelve a marcar por sí mismo tras perder el enlace, y cuya propia fila n2n 'Sessions'\nsignifica lo mismo. Desactivado afirma que UN solo Next apunta a este PC: dos Next\napuntando aquí se expulsarían mutuamente, y un Next distinto que se conecte se\ntrata como el mismo — déjalo activado siempre que puedan conectarse más de uno.\nSi se desactiva con varios Next conectados, el siguiente que se conecte los sustituye a todos.\nSe lee cuando un Next se conecta, así que un cambio se aplica a la siguiente conexión.\nLas sincronizaciones '.sync5' clásicas no se ven afectadas. Se guarda en el archivo de configuración.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Explorador remoto: {address} se ha conectado mientras {old} ocupaba la sesión — Sesiones está desactivado, así que se toma por el mismo Next que vuelve: se cierra el enlace antiguo y el recién llegado ocupa su lugar.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "el Next se ha vuelto a conectar mientras había un comando en curso — Sesiones está desactivado, así que se cerró el enlace antiguo y ese comando se perdió",
    ...
    "pt": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Sessões: aceitar vários Next ao mesmo tempo (Explorador remoto)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Ligado (predefinição): o servidor NextSync do Explorador remoto aceita até quatro Next\na executar '.sync5 -listen' ao mesmo tempo — a lista de máquinas e os separadores de sessão\nescolhem qual o painel controla, e um quinto é recusado com Busy.\nDesligado: exatamente UMA sessão. Um Next que se liga enquanto outro ocupa a sessão é tomado\npela MESMA máquina a regressar após uma ligação morta (o módulo ESP reiniciou,\no Wi-Fi falhou): a ligação antiga é cortada e o recém-chegado ocupa o seu\nlugar de imediato — mesma sessão, mesmo painel, sem reinício — em vez de esperar\natrás da ligação morta até expirar. Um comando cortado pela queda é\nrepetido até três vezes, com três segundos de pausa, assim que o Next volta (o\nregisto mostra retry1, retry2…); o resto de uma cópia em fila continua pela\nnova. Combine-o com o ZX Next Remote 1.2.5 ou mais recente, cujo Listener\nvolta a ligar por si só após perder a ligação, e cuja própria linha n2n 'Sessions'\nsignifica o mesmo. Desligado afirma que UM só Next aponta para este PC: dois Next\napontados para aqui expulsar-se-iam um ao outro, e um Next diferente que se ligue é\ntratado como o mesmo — deixe-o ligado sempre que mais do que um possa ligar-se.\nSe for desligado com vários Next ligados, o próximo a ligar-se substitui-os a todos.\nLido quando um Next se liga, pelo que uma alteração se aplica à ligação seguinte.\nAs sincronizações '.sync5' clássicas não são afetadas. Guardado no ficheiro de configuração.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Explorador remoto: {address} ligou-se enquanto {old} ocupava a sessão — Sessões está desligado, por isso é tomado pelo mesmo Next a regressar: a ligação antiga é cortada e o recém-chegado ocupa o seu lugar.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "o Next voltou a ligar-se enquanto um comando estava em curso — Sessões está desligado, por isso a ligação antiga foi cortada e esse comando perdeu-se",
    ...
    "pl": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Sesje: obsługuj kilka Nextów naraz (Zdalny eksplorator)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Włączone (domyślnie): serwer NextSync Zdalnego eksploratora obsługuje do czterech Nextów\nz uruchomionym '.sync5 -listen' naraz — lista maszyn i karty sesji\nwybierają, którym steruje panel, a piąty jest odrzucany jako Busy.\nWyłączone: dokładnie JEDNA sesja. Next, który łączy się, gdy inny zajmuje sesję, jest uznawany\nza TĘ SAMĄ maszynę wracającą po zerwanym łączu (jej moduł ESP się zresetował,\njej Wi-Fi mrugnęło): stare łącze zostaje zerwane, a nowy zajmuje jego\nmiejsce od razu — ta sama sesja, ten sam panel, bez resetu — zamiast czekać\nza martwym łączem, aż wygaśnie. Polecenie przerwane przez zerwanie jest\nponawiane do trzech razy, co trzy sekundy, gdy tylko Next wróci (w dzienniku\nwidać retry1, retry2…); reszta zakolejkowanej kopii toczy się dalej\npo nowym. Łącz to z ZX Next Remote 1.2.5 lub nowszym, którego Listener\nsam dzwoni ponownie po utracie łącza i którego własny wiersz n2n 'Sessions'\nznaczy to samo. Wyłączone stwierdza, że na ten PC celuje JEDEN Next: dwa Nexty\nskierowane tutaj wyrzucałyby się nawzajem, a inny Next, który się połączy, jest\ntraktowany jak ten sam — zostaw włączone, gdy może połączyć się więcej niż jeden.\nWyłączone, gdy zajętych jest kilka sesji: następny Next, który się połączy, zastępuje je wszystkie.\nOdczytywane, gdy Next się łączy, więc zmiana obowiązuje od następnego połączenia.\nKlasyczne synchronizacje '.sync5' pozostają bez zmian. Zapisywane w pliku konfiguracji.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Zdalny eksplorator: {address} połączył się, gdy {old} zajmował sesję — Sesje są wyłączone, więc uznano go za ten sam Next wracający do nas: stare łącze zostaje zerwane, a nowy zajmuje jego miejsce.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "Next połączył się ponownie, gdy polecenie było w toku — Sesje są wyłączone, więc stare łącze zostało zerwane, a to polecenie przepadło",
    ...
    "ru": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Сессии: принимать несколько Next одновременно (удалённый проводник)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Вкл (по умолчанию): сервер NextSync удалённого проводника принимает до четырёх Next\nс запущенным '.sync5 -listen' одновременно — список машин и вкладки сессий\nвыбирают, какой из них управляет панель, а пятому отвечают Busy.\nВыкл: ровно ОДНА сессия. Next, подключившийся, пока сессию занимает другой, считается\nТОЙ ЖЕ машиной, вернувшейся после мёртвого соединения (её модуль ESP перезапустился,\nеё Wi-Fi моргнул): старое соединение разрывается, и новый сразу занимает его\nместо — та же сессия, та же панель, без сброса — вместо ожидания\nза мёртвым соединением до истечения таймаута. Команда, прерванная обрывом,\nповторяется до трёх раз с паузой в три секунды, как только Next вернётся (в журнале\nвидны retry1, retry2…); остаток очереди копирования продолжается по\nновому. Сочетайте это с ZX Next Remote 1.2.5 или новее, чей Listener\nсам перезванивает после потери соединения и чья собственная строка n2n 'Sessions'\nозначает то же самое. Выкл утверждает, что на этот ПК нацелен ОДИН Next: два Next,\nнаправленные сюда, вытесняли бы друг друга, а другой подключившийся Next\nсчитается тем же самым — оставляйте Вкл, если подключиться может больше одного.\nЕсли выключить, когда подключено несколько Next, следующий подключившийся заменит их все.\nЧитается при подключении Next, так что изменение действует со следующего подключения.\nКлассические синхронизации '.sync5' не затрагиваются. Сохраняется в файле конфигурации.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Удалённый проводник: {address} подключился, пока сессию занимал {old} — Сессии выключены, поэтому он считается тем же вернувшимся Next: старое соединение разрывается, и новый занимает его место.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "Next подключился снова, пока выполнялась команда — Сессии выключены, поэтому старое соединение было разорвано, а эта команда потеряна",
    ...
    "cs": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Relace: obsluhovat několik Nextů najednou (Vzdálený průzkumník)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Zapnuto (výchozí): server NextSync Vzdáleného průzkumníka obslouží až čtyři Nexty\nse spuštěným '.sync5 -listen' najednou — seznam strojů a karty relací\nvolí, který z nich panel řídí, a pátý je odmítnut jako Busy.\nVypnuto: přesně JEDNA relace. Next, který se připojí, zatímco relaci drží jiný, je považován\nza TENTÝŽ stroj vracející se po mrtvém spojení (jeho modul ESP se resetoval,\njeho Wi-Fi zablikala): staré spojení se zruší a nově příchozí zaujme jeho\nmísto okamžitě — stejná relace, stejný panel, bez resetu — místo čekání\nza mrtvým spojením, než vyprší. Příkaz přerušený výpadkem se\nopakuje až třikrát, s třísekundovou pauzou, jakmile je Next zpět (v protokolu\nje vidět retry1, retry2…); zbytek zařazené kopie pokračuje po\nnovém. Kombinujte to se ZX Next Remote 1.2.5 nebo novějším, jehož Listener\npo ztrátě spojení sám znovu vytáčí a jehož vlastní řádek n2n 'Sessions'\nznamená totéž. Vypnuto tvrdí, že na toto PC míří JEDEN Next: dva Nexty\nnamířené sem by se vzájemně vypuzovaly a jiný Next, který se připojí, je\nbrán jako tentýž — nechte zapnuto, kdykoli se může připojit více než jeden.\nVypnete-li to, když je připojeno několik Nextů, další, který se připojí, je nahradí všechny.\nČte se při připojení Nextu, takže změna platí od dalšího připojení.\nKlasické synchronizace '.sync5' nejsou dotčeny. Ukládá se do konfiguračního souboru.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Vzdálený průzkumník: {address} se připojil, zatímco relaci držel {old} — Relace jsou vypnuté, takže je považován za tentýž Next, který se vrací: staré spojení se zruší a nově příchozí zaujme jeho místo.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "Next se znovu připojil, zatímco probíhal příkaz — Relace jsou vypnuté, takže staré spojení bylo zrušeno a tento příkaz se ztratil",
    ...
    "fr": {
        ...
        "NextSync — Sessions: seat several Nexts at once (Remote Explorer)": "NextSync — Sessions : accueillir plusieurs Next à la fois (Explorateur distant)",
        "On (default): the Remote Explorer NextSync server seats up to four Nexts\nrunning '.sync5 -listen' at once — the machine list and the session tabs\nswitch which one the pane drives, and a fifth is turned away as Busy.\nOff: exactly ONE seat. A Next that dials in while one is seated is taken\nfor the SAME machine coming back over a dead link (its ESP module reset,\nits Wi-Fi blinked): the old link is dropped and the newcomer takes its\nplace at once — same session, same pane, no reset — instead of waiting\nbehind the dead link until it times out. A command cut by the drop is\nretried up to three times, three seconds apart, once the Next is back (the\nlog shows retry1, retry2…); the rest of a queued copy carries on over the\nnew one. Pair it with ZX Next Remote 1.2.5 or newer, whose Listener\nre-dials by itself after a lost link, and whose own n2n 'Sessions' row\nmeans the same thing. Off asserts that ONE Next targets this PC: two Nexts\npointed here would evict each other, and a different Next dialing in is\ntreated as the same one — leave it On whenever more than one may connect.\nTurned Off while several are seated, the next Next to dial in replaces them all.\nRead when a Next dials in, so a change applies to the next connection.\nClassic '.sync5' syncs are not affected. Saved to the configuration file.": "Activé (par défaut) : le serveur NextSync de l'Explorateur distant accueille jusqu'à quatre Next\nexécutant '.sync5 -listen' à la fois — la liste des machines et les onglets de session\nchoisissent laquelle le panneau pilote, et un cinquième est refusé avec Busy.\nDésactivé : exactement UNE session. Un Next qui se connecte alors qu'un autre occupe la session est pris\npour la MÊME machine qui revient après un lien mort (son module ESP s'est réinitialisé,\nson Wi-Fi a cligné) : l'ancien lien est coupé et le nouveau venu prend sa\nplace aussitôt — même session, même panneau, sans réinitialisation — au lieu d'attendre\nderrière le lien mort jusqu'à son expiration. Une commande coupée par la perte est\nretentée jusqu'à trois fois, à trois secondes d'intervalle, dès que le Next est de retour (le\njournal affiche retry1, retry2…) ; le reste d'une copie en file continue sur\nle nouveau. Associez-le à ZX Next Remote 1.2.5 ou plus récent, dont le Listener\nrecompose de lui-même après un lien perdu, et dont la propre ligne n2n 'Sessions'\nsignifie la même chose. Désactivé affirme qu'UN seul Next vise ce PC : deux Next\npointés ici s'évinceraient l'un l'autre, et un Next différent qui se connecte est\ntraité comme le même — laissez-le activé dès que plus d'un peut se connecter.\nDésactivé alors que plusieurs Next sont connectés, le prochain à se connecter les remplace tous.\nLu quand un Next se connecte, un changement s'applique donc à la connexion suivante.\nLes synchronisations '.sync5' classiques ne sont pas concernées. Enregistré dans le fichier de configuration.",
        "Remote explorer: {address} dialed in while {old} was seated — Sessions is Off, so it is taken for the same Next coming back: the old link is dropped and the newcomer takes its place.": "Explorateur distant : {address} s'est connecté alors que {old} occupait la session — Sessions est désactivé, il est donc pris pour le même Next qui revient : l'ancien lien est coupé et le nouveau venu prend sa place.",
        "the Next dialed in again while a command was in flight — Sessions is Off, so the old link was dropped and that command was lost": "le Next s'est reconnecté alors qu'une commande était en cours — Sessions est désactivé, l'ancien lien a donc été coupé et cette commande est perdue",
